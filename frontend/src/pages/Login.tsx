...
export default function Login({
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-bg-base px-4 relative overflow-hidden">
      ...
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-blue-400/20 blur-[100px] rounded-full z-0 pointer-events-none"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-green-400/20 blur-[100px] rounded-full z-0 pointer-events-none"></div>

      <div className="w-full max-w-md glass rounded-3xl shadow-premium p-8 relative z-10">
        ...
        <form className="space-y-6 mt-8" onSubmit={handleSubmit}>
          ...
          <label className="flex items-center gap-2.5 text-sm font-medium text-gray-700 cursor-pointer group w-fit">
            <div className="relative flex items-center">
              <input
                type="checkbox"
                checked={rememberMe}
                onChange={(e) => setRememberMe(e.target.checked)}
                className="peer appearance-none w-5 h-5 border border-gray-300 rounded cursor-pointer transition-all checked:bg-[#0a5ea8] checked:border-transparent focus-ring"
              />
              <svg
                className="absolute w-3 h-3 pointer-events-none hidden peer-checked:block left-1 top-1 text-white"
                viewBox="0 0 14 14"
                fill="none"
              >
                <path
          ...
          <button
            ...
            className="w-full flex justify-center items-center py-3.5 px-4 rounded-xl shadow-md text-lg font-bold text-white bg-[#0a5ea8] hover:bg-[#084d8a] disabled:opacity-70 transition-all group"
          >
